...
from config.dir_config import HTML_DIR, make_dir, TEMPLATE_FILE
...
class GitPrey(object):
    ...
    @staticmethod
    def __page_project_info_list(page_html, i, kw):
        """
        Get project list of one searching result page
        :param page_html: Html page content
        :returns: Project list of per page
        ...
        project_info = cur_par_html.select("div.width-full")
        ...
        for project in project_info:
            ...
            p = {}
            repo = project.select_one("div.flex-shrink-0.text-small.text-bold > a")
            repo_file = project.select_one("div.f4.text-normal > a")
            blob_num = project.select_one("td.blob-num > a")
            blob_html = project.select_one("div.file-box.blob-wrapper.my-1 > table")
            if repo is not None:
                p["repo_name"] = str(repo.text).strip()
                p["repo_url"] = HOST_NAME[0:-1] + repo.get("href", "")
                p["file_name"] = str(repo_file.text).strip()
                p["file_url"] = HOST_NAME[0:-1] + repo_file.get("href", "")
...
def project_miner(key_words):
    ...
    total_project_list = _gitprey.search_repos()
...
if __name__ == "__main__":
    ...
    kws = get_file("keywords.txt")
    infos = get_file(INFO_DB)
    global dbsession
    c = get_template_file()
    print(c)
    for kw in kws:
        KW_BASE_DIR = os.path.join(HTML_DIR, kw)
        make_dir(KW_BASE_DIR)
        dbsession = BaseRepository(collectionname=kw)
        for info in infos:
            INFO_BASE_DIR = os.path.join(KW_BASE_DIR, info)
            INFO_HUAWEI_BASE_DIR = os.path.join(KW_BASE_DIR, info + "_huawei")
            make_dir(INFO_BASE_DIR)
            make_dir(INFO_HUAWEI_BASE_DIR)
            keyword = kw + " " + info
            debug_print("开始搜索关键字:" + keyword)
            _gitprey = GitPrey(keyword)
            total_project_list, h = _gitprey.search_repos()
            z = c.replace("{{content}}", h)
            # Scanning deep according to page searching count and time out seconds

            save_file = os.path.join(INFO_BASE_DIR, "1-" + str(SCAN_DEEP[SEARCH_LEVEL - 1]) + ".html")
            with open(save_file, "w", encoding="utf-8") as f:
                f.write(z)

            debug_print("开始搜索关键字:" + keyword + " huawei")
            _gitprey = GitPrey(keyword + " huawei")
            total_project_list, h = _gitprey.search_repos()
            z = c.replace("{{content}}", h)
            save_file = os.path.join(INFO_HUAWEI_BASE_DIR, "1-" + str(SCAN_DEEP[SEARCH_LEVEL - 1]) + ".html")
            with open(save_file, "w", encoding="utf-8") as f:
                f.write(z)
